...
function Get-SystemHealth {
    ...
    try {
        ...
        $diskInfo = @{ Name = "Nao detectado"; Score = 0; Status = "Critico"; Alerts = @("Nenhum disco detectado"); HealthStatus = "Erro"; Size = "N/D" }

        if ($disks) {
            $worstScore = 101
            $worstDisk = $null
            foreach ($disk in $disks) {
                $mediaType = switch ($disk.MediaType) { 4 { "SSD" } 3 { "HDD" } default { "Disco" } }
                $sizeGB = [math]::Round($disk.Size / 1GB, 0)
                $dScore = 100; $dStatus = "Bom"; $dAlerts = @(); $dHealth = "Saudavel"

                switch ([int]$disk.HealthStatus) {
                    0 { $dHealth = "Saudavel"; $dScore = 100 }
                    1 { $dHealth = "Aviso"; $dScore = 40; $dStatus = "Atencao"; $dAlerts += "$mediaType $($disk.FriendlyName): SMART indica degradacao - faca backup!" }
                    2 { $dHealth = "Critico"; $dScore = 10; $dStatus = "Critico"; $dAlerts += "$mediaType $($disk.FriendlyName): SMART indica falha iminente!" }
                    default { $dHealth = "Desconhecido"; $dScore = 70; $dStatus = "Atencao" }
                }

                if ($dScore -lt $worstScore) {
                    $worstScore = $dScore
                    $worstDisk = @{
                        Name = "$mediaType - $($disk.FriendlyName)"; Score = $dScore; Status = $dStatus
                        Alerts = [System.Collections.ArrayList]@($dAlerts)
                        HealthStatus = $dHealth; Size = "${sizeGB} GB"; MediaType = $mediaType
                    }
                }
            }
            ...
            foreach ($ld in $logicalDisks) {
                if ($ld.Size -and $ld.Size -gt 0) {
                    $freePercent = [math]::Round(($ld.FreeSpace / $ld.Size) * 100, 1)
                    $freeGB = [math]::Round($ld.FreeSpace / 1GB, 1)
                    if ($freePercent -lt 10) {
                        $worstDisk.Score = [math]::Min($worstDisk.Score, 20)
                        $worstDisk.Status = "Critico"
                        $worstDisk.Alerts.Add("$($ld.DeviceID) apenas ${freeGB}GB livre ($freePercent%)") | Out-Null
                    } elseif ($freePercent -lt 20) {
                        $worstDisk.Score = [math]::Min($worstDisk.Score, 55)
                        if ($worstDisk.Status -eq "Bom") { $worstDisk.Status = "Atencao" }
                        $worstDisk.Alerts.Add("$($ld.DeviceID) com ${freeGB}GB livre ($freePercent%)") | Out-Null
                    }
                }
            }
            if ($worstDisk) { $diskInfo = $worstDisk }
        }
        $health.Disco = $diskInfo
    } catch {
        $health.Disco = @{ Name = "Erro na leitura"; Score = 0; Status = "Critico"; Alerts = @("Erro: $_"); HealthStatus = "Erro"; Size = "N/D" }
    }
    ...
    try {
        ...
        $cpuScore = 100; $cpuStatus = "Bom"; $cpuAlerts = @()

        if ($cpuLoad -gt 90) { $cpuScore = 15; $cpuStatus = "Critico"; $cpuAlerts += "Carga acima de 90% ($cpuLoad%)" }
        elseif ($cpuLoad -gt 70) { $cpuScore = 45; $cpuStatus = "Atencao"; $cpuAlerts += "Carga elevada ($cpuLoad%)" }
        else { $cpuScore = [math]::Max(60, 100 - $cpuLoad) }
        ...
        try {
            ...
            if ($thermal -and $thermal.CurrentTemperature) {
                $cpuTemp = [math]::Round(($thermal.CurrentTemperature / 10) - 273.15, 1)
                if ($cpuTemp -gt 85) { $cpuScore = [math]::Min($cpuScore, 15); $cpuStatus = "Critico"; $cpuAlerts += "Temperatura critica: ${cpuTemp}C" }
                elseif ($cpuTemp -gt 70) { $cpuScore = [math]::Min($cpuScore, 45); if ($cpuStatus -eq "Bom") { $cpuStatus = "Atencao" }; $cpuAlerts += "Temperatura elevada: ${cpuTemp}C" }
            }
        } catch {}

        $cpuName = if ($cpu.Name) { $cpu.Name.Trim() } else { "Desconhecida" }
        $health.CPU = @{
            Name = $cpuName; Cores = "$($cpu.NumberOfCores)C/$($cpu.NumberOfLogicalProcessors)T"
            Speed = "$([math]::Round($cpu.MaxClockSpeed / 1000, 2)) GHz"; Load = "$cpuLoad%"
            Temperature = if ($cpuTemp) { "${cpuTemp}C" } else { "N/D" }
            Score = $cpuScore; Status = $cpuStatus; Alerts = $cpuAlerts
        }
    } catch {
        $health.CPU = @{ Name = "Erro"; Score = 0; Status = "Critico"; Alerts = @("Erro: $_"); Load = "N/D"; Cores = "N/D"; Speed = "N/D"; Temperature = "N/D" }
    }
    ...
    try {
        ...
        $usedGB = [math]::Round($totalGB - $freeGB, 2)
        $pctUsed = [math]::Round(($usedGB / $totalGB) * 100, 1)
        $ramScore = 100; $ramStatus = "Bom"; $ramAlerts = @()

        if ($pctUsed -gt 90) { $ramScore = 10; $ramStatus = "Critico"; $ramAlerts += "RAM acima de 90%! Feche aplicativos." }
        elseif ($pctUsed -gt 70) { $ramScore = 45; $ramStatus = "Atencao"; $ramAlerts += "RAM elevada ($pctUsed%)" }
        else { $ramScore = [math]::Max(55, 100 - [int]$pctUsed) }

        $ramModules = Get-CimInstance -ClassName Win32_PhysicalMemory -ErrorAction SilentlyContinue
        $ramSpeed = if ($ramModules -and @($ramModules).Count -gt 0) { "$(@($ramModules)[0].Speed) MHz" } else { "N/D" }

        $health.RAM = @{
            Name = "${totalGB} GB RAM"; Used = "${usedGB} GB ($pctUsed%)"; Free = "${freeGB} GB"
            Speed = $ramSpeed; PctUsed = $pctUsed; Score = $ramScore; Status = $ramStatus; Alerts = $ramAlerts
        }
    } catch {
        $health.RAM = @{ Name = "Erro"; Score = 0; Status = "Critico"; Alerts = @("Erro: $_"); Used = "N/D"; Free = "N/D"; Speed = "N/D"; PctUsed = 0 }
    }

    # -- GPU --
    try {
        $gpus = Get-CimInstance -ClassName Win32_VideoController -ErrorAction SilentlyContinue
        $gpu = @($gpus) | Where-Object { $_.AdapterRAM -and $_.AdapterRAM -gt 0 } | Select-Object -First 1
        if (-not $gpu) { $gpu = @($gpus) | Select-Object -First 1 }
        $gpuScore = 100; $gpuStatus = "Bom"; $gpuAlerts = @()

        if ($gpu) {
            if ($gpu.Status -and $gpu.Status -ne "OK") {
                $gpuScore = 35; $gpuStatus = "Atencao"; $gpuAlerts += "Status do driver: $($gpu.Status)"
            }
            $vram = if ($gpu.AdapterRAM -and $gpu.AdapterRAM -gt 0) { "$([math]::Round($gpu.AdapterRAM / 1GB, 1)) GB" } else { "Integrada" }
            $gpuName = if ($gpu.Name) { $gpu.Name } else { "Desconhecida" }
            $health.GPU = @{ Name = $gpuName; VRAM = $vram; DriverVersion = $(if ($gpu.DriverVersion) { $gpu.DriverVersion } else { "N/D" }); Score = $gpuScore; Status = $gpuStatus; Alerts = $gpuAlerts }
        } else {
            $health.GPU = @{ Name = "Nao detectada"; VRAM = "N/D"; DriverVersion = "N/D"; Score = 50; Status = "Atencao"; Alerts = @("GPU nao detectada") }
        }
    } catch {
        $health.GPU = @{ Name = "Erro"; VRAM = "N/D"; DriverVersion = "N/D"; Score = 0; Status = "Critico"; Alerts = @("Erro: $_") }
    }
    ...
    try {
        ...
        if ($battery) {
            $charge = if ($battery.EstimatedChargeRemaining) { [int]$battery.EstimatedChargeRemaining } else { 0 }
            $batScore = $charge; $batStatus = "Bom"; $batAlerts = @()
            $statusText = switch ([int]$battery.BatteryStatus) {
                1 { "Descarregando" } 2 { "Conectado (AC)" } 3 { "Carregado" }
                4 { "Baixa" } 5 { "Critica" } default { "Desconhecido" }
            }
            if ($charge -lt 20) { $batStatus = "Critico"; $batAlerts += "Bateria abaixo de 20%!" }
            elseif ($charge -lt 50) { $batStatus = "Atencao"; $batAlerts += "Bateria abaixo de 50%" }
            $health.Bateria = @{ Name = "Bateria"; Charge = "$charge%"; StatusText = $statusText; Score = $batScore; Status = $batStatus; Alerts = $batAlerts; Available = $true }
        } else {
            $health.Bateria = @{ Name = "Desktop"; Charge = "N/A"; StatusText = "Sem bateria"; Score = 100; Status = "Bom"; Alerts = @(); Available = $false }
        }
    } catch {
        $health.Bateria = @{ Name = "Erro"; Charge = "N/A"; StatusText = "Erro"; Score = 0; Status = "Critico"; Alerts = @("Erro: $_"); Available = $false }
    }

    # -- Rede --
    try {
        $adapters = Get-CimInstance -ClassName Win32_NetworkAdapter -Filter "NetConnectionStatus=2" -ErrorAction SilentlyContinue
        $netScore = 100; $netStatus = "Bom"; $netAlerts = @()
        if (-not $adapters -or @($adapters).Count -eq 0) {
            $netScore = 0; $netStatus = "Critico"; $netAlerts += "Sem conexao de rede!"
            $health.Rede = @{ Name = "Desconectado"; Speed = "N/D"; Adapters = 0; Score = $netScore; Status = $netStatus; Alerts = $netAlerts }
        } else {
            $primary = @($adapters) | Select-Object -First 1
            $adapterName = if ($primary.Name) { $primary.Name } else { "Adaptador" }
            $speed = if ($primary.Speed -and $primary.Speed -gt 0) { "$([math]::Round($primary.Speed / 1000000, 0)) Mbps" } else { "N/D" }
            if ($primary.Speed -and $primary.Speed -gt 0 -and $primary.Speed -lt 100000000) {
                $netScore = 50; $netStatus = "Atencao"; $netAlerts += "Velocidade baixa: $speed"
            }
            $health.Rede = @{ Name = $adapterName; Speed = $speed; Adapters = @($adapters).Count; Score = $netScore; Status = $netStatus; Alerts = $netAlerts }
        }
    } catch {
        $health.Rede = @{ Name = "Erro"; Speed = "N/D"; Adapters = 0; Score = 0; Status = "Critico"; Alerts = @("Erro: $_") }
    }
...
$tabControl.ItemSize = New-Object System.Drawing.Size(267, 36)
...
$tabHealth.Text = "Health Status"
$tabHealth.BackColor = $bgDark
$tabHealth.ForeColor = $textPrimary
$tabHealth.Padding = New-Object System.Windows.Forms.Padding(0)
...
# -- Health Header --
$healthHeaderPanel = New-Object System.Windows.Forms.Panel
$healthHeaderPanel.Location = New-Object System.Drawing.Point(20, 10)
$healthHeaderPanel.Size = New-Object System.Drawing.Size(762, 55)
$healthHeaderPanel.BackColor = $bgPanel

$healthHeaderLabel = New-Object System.Windows.Forms.Label
$healthHeaderLabel.Text = "[+] Health Status"
$healthHeaderLabel.Font = $fontTitle
$healthHeaderLabel.ForeColor = $accentGreen
$healthHeaderLabel.Location = New-Object System.Drawing.Point(16, 6)
$healthHeaderLabel.AutoSize = $true
$healthHeaderPanel.Controls.Add($healthHeaderLabel)

$healthSubLabel = New-Object System.Windows.Forms.Label
$healthSubLabel.Text = "Monitoramento de saude dos componentes do PC"
$healthSubLabel.Font = $fontSmall
$healthSubLabel.ForeColor = $textSecondary
$healthSubLabel.Location = New-Object System.Drawing.Point(18, 34)
$healthSubLabel.AutoSize = $true
$healthHeaderPanel.Controls.Add($healthSubLabel)

$tabHealth.Controls.Add($healthHeaderPanel)

# -- Health Card Helper --
function New-HealthCard {
    param([int]$x, [int]$y, [string]$compLabel)

    $card = New-Object System.Windows.Forms.Panel
    $card.Location = New-Object System.Drawing.Point($x, $y)
    $card.Size = New-Object System.Drawing.Size(245, 75)
    $card.BackColor = $bgCard

    $indicator = New-Object System.Windows.Forms.Label
    $indicator.Name = "indicator"
    $indicator.Text = [string][char]0x25CF
    $indicator.Font = New-Object System.Drawing.Font("Segoe UI", 14)
    $indicator.ForeColor = $textSecondary
    $indicator.Location = New-Object System.Drawing.Point(10, 6)
    $indicator.AutoSize = $true
    $card.Controls.Add($indicator)

    $lblTitle = New-Object System.Windows.Forms.Label
    $lblTitle.Name = "title"
    $lblTitle.Text = $compLabel
    $lblTitle.Font = $fontSubtitle
    $lblTitle.ForeColor = $textPrimary
    $lblTitle.Location = New-Object System.Drawing.Point(36, 8)
    $lblTitle.AutoSize = $true
    $card.Controls.Add($lblTitle)

    $lblSub = New-Object System.Windows.Forms.Label
    $lblSub.Name = "subtitle"
    $lblSub.Text = "Aguardando verificacao..."
    $lblSub.Font = $fontSmall
    $lblSub.ForeColor = $textSecondary
    $lblSub.Location = New-Object System.Drawing.Point(36, 30)
    $lblSub.Size = New-Object System.Drawing.Size(200, 16)
    $card.Controls.Add($lblSub)

    $lblScore = New-Object System.Windows.Forms.Label
    $lblScore.Name = "score"
    $lblScore.Text = "-- %"
    $lblScore.Font = $fontMedium
    $lblScore.ForeColor = $textSecondary
    $lblScore.Location = New-Object System.Drawing.Point(36, 49)
...
# Create 6 health cards (3 x 2 grid)
$script:hCardDisco   = New-HealthCard 20  75  "DISCO"
$script:hCardCPU     = New-HealthCard 275 75  "CPU"
$script:hCardRAM     = New-HealthCard 530 75  "RAM"
$script:hCardGPU     = New-HealthCard 20  158 "GPU"
$script:hCardBateria = New-HealthCard 275 158 "BATERIA"
$script:hCardRede    = New-HealthCard 530 158 "REDE"

$tabHealth.Controls.AddRange(@($script:hCardDisco, $script:hCardCPU, $script:hCardRAM, $script:hCardGPU, $script:hCardBateria, $script:hCardRede))

# -- Verify Health Button --
$btnVerifyHealth = New-Object System.Windows.Forms.Button
$btnVerifyHealth.Text = "VERIFICAR SAUDE"
$btnVerifyHealth.Font = $fontMedium
$btnVerifyHealth.Size = New-Object System.Drawing.Size(762, 42)
$btnVerifyHealth.Location = New-Object System.Drawing.Point(20, 243)
$btnVerifyHealth.FlatStyle = "Flat"
$btnVerifyHealth.FlatAppearance.BorderSize = 0
$btnVerifyHealth.BackColor = $accentGreen
$btnVerifyHealth.ForeColor = [System.Drawing.Color]::FromArgb(20, 20, 30)
$btnVerifyHealth.Cursor = [System.Windows.Forms.Cursors]::Hand
$tabHealth.Controls.Add($btnVerifyHealth)

$btnVerifyHealth.Add_MouseEnter({ $this.BackColor = [System.Drawing.Color]::FromArgb(60, 200, 120) })
$btnVerifyHealth.Add_MouseLeave({ if ($this.Enabled) { $this.BackColor = $accentGreen } })

# -- Chart Section --
$healthChartTitle = New-Object System.Windows.Forms.Label
$healthChartTitle.Text = "[=] Relatorio de Saude"
$healthChartTitle.Font = $fontSubtitle
$healthChartTitle.ForeColor = $textPrimary
$healthChartTitle.Location = New-Object System.Drawing.Point(20, 295)
$healthChartTitle.AutoSize = $true
$tabHealth.Controls.Add($healthChartTitle)

$healthChartPanel = New-Object System.Windows.Forms.Panel
$healthChartPanel.Location = New-Object System.Drawing.Point(20, 318)
$healthChartPanel.Size = New-Object System.Drawing.Size(762, 175)
$healthChartPanel.BackColor = $bgCard

# Enable double buffering to prevent flicker
$healthChartPanel.GetType().GetProperty("DoubleBuffered", [System.Reflection.BindingFlags]::Instance -bor [System.Reflection.BindingFlags]::NonPublic).SetValue($healthChartPanel, $true, $null)

$healthChartPanel.Add_Paint({
    $g = $_.Graphics
    $g.SmoothingMode = [System.Drawing.Drawing2D.SmoothingMode]::AntiAlias
    $g.TextRenderingHint = [System.Drawing.Text.TextRenderingHint]::ClearTypeGridFit

    $components = @("Disco", "CPU", "RAM", "GPU", "Bateria", "Rede")
    $barHeight = 22
    $barGap = 6
    $labelWidth = 80
    $barStartX = $labelWidth + 15
    $barMaxWidth = $this.Width - $barStartX - 125
    $startY = 10

    $labelFont = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    $scoreFont = New-Object System.Drawing.Font("Segoe UI", 8)
    $textBrush = New-Object System.Drawing.SolidBrush($textPrimary)
    $bgBarBrush = New-Object System.Drawing.SolidBrush($bgPanel)

    for ($i = 0; $i -lt $components.Count; $i++) {
        $compKey = $components[$i]
        $y = $startY + ($i * ($barHeight + $barGap))

        $score = 0; $status = ""
        if ($script:healthData -and $script:healthData.ContainsKey($compKey)) {
            $score = [int]$script:healthData[$compKey].Score
            $status = $script:healthData[$compKey].Status
        }

        $g.DrawString($compKey, $labelFont, $textBrush, 12, ($y + 2))

        $g.FillRectangle($bgBarBrush, $barStartX, $y, $barMaxWidth, $barHeight)

        $fillWidth = [math]::Max(1, [int]($barMaxWidth * $score / 100))
        $barColor = switch ($status) {
            "Bom"     { $accentGreen }
            "Atencao" { $accentYellow }
            "Critico" { $accentRed }
            default   { $textSecondary }
        }
        $fillBrush = New-Object System.Drawing.SolidBrush($barColor)
        $g.FillRectangle($fillBrush, $barStartX, $y, $fillWidth, $barHeight)
        $fillBrush.Dispose()
        ...
        $scoreText = if ($script:healthData) { "$score%  [$statusLabel]" } else { "-- %" }
        $statusBrush = New-Object System.Drawing.SolidBrush($barColor)
        $g.DrawString($scoreText, $scoreFont, $statusBrush, ($barStartX + $barMaxWidth + 8), ($y + 3))
        $statusBrush.Dispose()
    }
...
$tabHealth.Controls.Add($healthChartPanel)

# -- Alerts Section --
$healthAlertsTitle = New-Object System.Windows.Forms.Label
$healthAlertsTitle.Text = "[!] Alertas e Avisos"
$healthAlertsTitle.Font = $fontSubtitle
$healthAlertsTitle.ForeColor = $textPrimary
$healthAlertsTitle.Location = New-Object System.Drawing.Point(20, 501)
$healthAlertsTitle.AutoSize = $true
$tabHealth.Controls.Add($healthAlertsTitle)

$healthAlertsBox = New-Object System.Windows.Forms.TextBox
$healthAlertsBox.Location = New-Object System.Drawing.Point(20, 523)
$healthAlertsBox.Size = New-Object System.Drawing.Size(762, 115)
$healthAlertsBox.Multiline = $true
$healthAlertsBox.ScrollBars = "Vertical"
$healthAlertsBox.ReadOnly = $true
$healthAlertsBox.BackColor = $bgPanel
$healthAlertsBox.ForeColor = $accentGreen
$healthAlertsBox.Font = New-Object System.Drawing.Font("Consolas", 9)
$healthAlertsBox.BorderStyle = "None"
$healthAlertsBox.Text = "[$(Get-Date -Format 'HH:mm:ss')] Clique em VERIFICAR SAUDE para iniciar o diagnostico.`r`n"
$tabHealth.Controls.Add($healthAlertsBox)

# -- Update Health Card Values --
function Update-SingleHealthCard {
    param($card, [string]$subtitle, [string]$status, [int]$score)
    foreach ($ctrl in $card.Controls) {
        switch ($ctrl.Name) {
            "indicator" {
                $ctrl.ForeColor = switch ($status) {
                    "Bom" { $accentGreen } "Atencao" { $accentYellow } "Critico" { $accentRed } default { $textSecondary }
                }
            }
            "subtitle" { $ctrl.Text = $subtitle }
            "score" {
                $ctrl.Text = "$score%"
                $ctrl.ForeColor = switch ($status) {
                    "Bom" { $accentGreen } "Atencao" { $accentYellow } "Critico" { $accentRed } default { $textSecondary }
...
# -- Master Health Update --
function Update-HealthDisplay {
    $healthAlertsBox.Clear()
    $healthAlertsBox.ForeColor = $accentGreen
    $healthAlertsBox.AppendText("[$(Get-Date -Format 'HH:mm:ss')] Verificando saude dos componentes...`r`n")
    [System.Windows.Forms.Application]::DoEvents()

    $script:healthData = Get-SystemHealth

    # Update cards
    $d = $script:healthData.Disco
    Update-SingleHealthCard $script:hCardDisco "$($d.Name) | $($d.Size)" $d.Status $d.Score

    $c = $script:healthData.CPU
    Update-SingleHealthCard $script:hCardCPU "Carga: $($c.Load) | $($c.Cores)" $c.Status $c.Score

    $r = $script:healthData.RAM
    Update-SingleHealthCard $script:hCardRAM "Uso: $($r.Used) | $($r.Speed)" $r.Status $r.Score

    $gp = $script:healthData.GPU
    Update-SingleHealthCard $script:hCardGPU "VRAM: $($gp.VRAM) | $($gp.DriverVersion)" $gp.Status $gp.Score

    $b = $script:healthData.Bateria
    if ($b.Available) {
        Update-SingleHealthCard $script:hCardBateria "Carga: $($b.Charge) | $($b.StatusText)" $b.Status $b.Score
    } else {
        Update-SingleHealthCard $script:hCardBateria "Desktop (sem bateria)" "Bom" 100
    }

    $n = $script:healthData.Rede
    Update-SingleHealthCard $script:hCardRede "$($n.Speed) | $($n.Adapters) adaptador(es)" $n.Status $n.Score

    # Refresh chart
    $healthChartPanel.Invalidate()
    ...
    foreach ($compName in @("Disco", "CPU", "RAM", "GPU", "Bateria", "Rede")) {
        $compData = $script:healthData[$compName]
        if ($compData -and $compData.Alerts -and @($compData.Alerts).Count -gt 0) {
            foreach ($alert in $compData.Alerts) {
                if ($compData.Status -eq "Critico") { $criticalAlerts += "[$compName] $alert" }
                else { $warningAlerts += "[$compName] $alert" }
            }
        }
    }

    if ($criticalAlerts.Count -gt 0) {
        $healthAlertsBox.ForeColor = $accentRed
        foreach ($a in $criticalAlerts) { $healthAlertsBox.AppendText("[$timestamp] [!!] CRITICO: $a`r`n") }
    }
    if ($warningAlerts.Count -gt 0) {
        if ($criticalAlerts.Count -eq 0) { $healthAlertsBox.ForeColor = $accentOrange }
        foreach ($a in $warningAlerts) { $healthAlertsBox.AppendText("[$timestamp] [>>] AVISO: $a`r`n") }
    }
    if ($criticalAlerts.Count -eq 0 -and $warningAlerts.Count -eq 0) {
        $healthAlertsBox.ForeColor = $accentGreen
        $healthAlertsBox.AppendText("[$timestamp] [OK] Todos os componentes estao saudaveis!`r`n")
    }

    # Overall score
    $overall = [math]::Round(( $script:healthData.Disco.Score + $script:healthData.CPU.Score + $script:healthData.RAM.Score + $script:healthData.GPU.Score + $script:healthData.Bateria.Score + $script:healthData.Rede.Score ) / 6, 0)
    $healthAlertsBox.AppendText("[$timestamp] Score geral do sistema: $overall%`r`n")
    $healthAlertsBox.SelectionStart = $healthAlertsBox.Text.Length
    $healthAlertsBox.ScrollToCaret()
    [System.Windows.Forms.Application]::DoEvents()
}

# -- Verify Button Click --
$btnVerifyHealth.Add_Click({
    $btnVerifyHealth.Enabled = $false
    $btnVerifyHealth.Text = "Verificando..."
    $btnVerifyHealth.BackColor = $bgCardHover
    [System.Windows.Forms.Application]::DoEvents()

    Update-HealthDisplay

    $btnVerifyHealth.Enabled = $true
    $btnVerifyHealth.Text = "VERIFICAR SAUDE"
    $btnVerifyHealth.BackColor = $accentGreen
})
...
# -- Health Tab: Auto-verify on first visit --
$tabControl.Add_SelectedIndexChanged({
    if ($tabControl.SelectedTab -eq $tabHealth -and $script:healthData -eq $null) {
        $btnVerifyHealth.Enabled = $false
        $btnVerifyHealth.Text = "Verificando..."
        $btnVerifyHealth.BackColor = $bgCardHover
        [System.Windows.Forms.Application]::DoEvents()
